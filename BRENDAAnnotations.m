function [AmodelAddedKEGG]=BRENDAAnnotations(Amodel,TabDelimFile,Row,Hits,MetPos)
                ...
                StrSearchCell=regexp(TabDelimFile(Row(Hits),:),'BRENDA-COMPOUND "[0-9]+"'); 
                ...
                if  ~isempty(TargetCellInRow)
                    ...
                    %looking for above.
                    FoundStr=regexprep(TabDelimFile{Row(Hits),TargetCellInRow},'.*BRENDA-COMPOUND "([0-9]+)".*','$1');
                    %Remember to change the target cell array to match the 
                    %customized annotation (Amodel.MetCustomAnnotation) 
                    Amodel.metBRENDA(MetPos,1)=cellstr(FoundStr);
                end
