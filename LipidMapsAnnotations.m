function [AmodelAddedKEGG]=LipidMapsAnnotations(Amodel,TabDelimFile,Row,Hits,MetPos)
                ...
                StrSearchCell=regexp(TabDelimFile(Row(Hits),:),'(LMFA\d{8}?)'); 
                ...
                if  ~isempty(TargetCellInRow)
                    ...
                    %looking for above.
                    FoundStr=regexprep(TabDelimFile{Row(Hits),TargetCellInRow},'.*(LMFA\d{8}?).*','$1');
                    %Remember to change the target cell array to match the 
                    %customized annotation (Amodel.MetCustomAnnotation) 
                    Amodel.metLipidMaps(MetPos,1)=cellstr(FoundStr);
                end
