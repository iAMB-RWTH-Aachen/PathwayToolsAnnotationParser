function [AmodelAddedKEGG]=ChemSpiderAnnotations(Amodel,TabDelimFile,Row,Hits,MetPos)
                ...
                StrSearchCell=regexp(TabDelimFile(Row(Hits),:),'CHEMSPIDER "[0-9]*"'); 
                ...
                if  ~isempty(TargetCellInRow)
                    ...
                    %looking for above.
                    FoundStr=regexprep(TabDelimFile{Row(Hits),TargetCellInRow},'.*CHEMSPIDER "([0-9]*)".*','$1');
                    %Remember to change the target cell array to match the 
                    %customized annotation (Amodel.MetCustomAnnotation) 
                    Amodel.metChemSpiderID(MetPos,1)=cellstr(FoundStr);
                end
